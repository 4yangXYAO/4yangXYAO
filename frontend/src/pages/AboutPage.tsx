...
export const AboutPage = () => {
   ...
   const educationTimeline = [
      {
         ...
         description: "Focused on networking, computer fundamentals, programming basics, and technical troubleshooting.",
         tags: ["TKJ", "NETWORKING"]
      },
      {
         ...
         description: "Applied learning in hardware/software troubleshooting and general technical problem-solving.",
   ...
   const organizationTimeline = [
      {
         ...
         description: "Led the student council and planned school events through leadership.",
         tags: ["LEADERSHIP", "ORG"]
      },
      {
         period: "2019 — 2022",
         role: "Library Member",
         company: "SMK 10 Nopember",
         description: "Managed book lending records and maintained the library ecosystem.",
         tags: ["ADMIN", "ARCHIVE"]
      }
   ];
   ...
   const displayCategories = [
      {
         category: "CORE",
         skills: ["JAVASCRIPT", "TYPESCRIPT", "REACT", "NEXT.JS", "TAILWIND", "HTML/CSS", "REDUX", "FRAMER MOTION", "PYTHON", "C++", "C"]
      },
      {
         category: "AI_MODELS",
      ...
      {
         category: "AI_AGENT",
         skills: ["AUTOGEN", "CLAUDE_CODE", "LANGGRAPH", "OPENAI_AGENT", "OPENCODE", "HERMES"]
      },
      {
         category: "ECOSYSTEM",
      ...
      >
         <div className="max-w-7xl mx-auto px-6 md:px-10">
            <div className="max-w-5xl mx-auto">
                     ...
                     <div className="lg:col-span-4 flex justify-end">
                        <div className="relative p-1 border border-[#00daf7]/20">
                           <img
                              ...
                              onError={(e) => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?w=400" }}
                           />
               ...
               <div className="overflow-hidden mb-32 group relative">
                  ...
                  >
                     ...
                     {[...careerTimeline, ...careerTimeline].map((item, index) => (
                        <div key={index} className="flex-shrink-0 w-[350px] md:w-[450px] flex">
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 mb-32">
                  <div>
                     <div className="flex items-center gap-4 mb-8 md:mb-12">
                        <h2 className="font-display text-3xl md:text-4xl text-white uppercase">ACADEMIC_ARCHIVE</h2>
                     </div>
                     <div className="space-y-8">
                        {educationTimeline.map((item, index) => (
                           <div key={index} className="relative p-6 border-l-2 border-[#00daf7]/30 bg-[#121212]/30 group hover:border-[#00daf7] transition-all">
                              <div className="font-mono text-[10px] text-[#00daf7] mb-2">{item.period}</div>
                              <h3 className="font-display text-xl text-white uppercase mb-1">{item.school}</h3>
                              <div className="font-mono text-[10px] text-gray-500 uppercase tracking-[0.2em] mb-4">{item.major}</div>
                  ...
                  <div>
                     <div className="flex items-center gap-4 mb-8 md:mb-12">
                        <h2 className="font-display text-3xl md:text-4xl text-white uppercase">ORG_NODES</h2>
                     </div>
                     <div className="space-y-8">
                        {organizationTimeline.map((item, index) => (
                           <div key={index} className="relative p-6 border-l-2 border-[#00daf7]/30 bg-[#121212]/30 group hover:border-[#00daf7] transition-all">
                              <div className="font-mono text-[10px] text-[#00daf7] mb-2">{item.period}</div>
                              <h3 className="font-display text-xl text-white uppercase mb-1">{item.role}</h3>
                              <div className="font-mono text-[10px] text-gray-500 uppercase tracking-[0.2em] mb-4">{item.company}</div>
                              <p className="font-sans text-xs text-gray-400 group-hover:text-gray-300 transition-colors">
                                 {item.description}
                              </p>
                           </div>
               ...
               {/* AI / Stack Section */}
               <div id="stack" className="mt-48 pt-24 border-t border-white/5 scroll-mt-24">
                  <div className="flex items-center gap-4 mb-16">
                     <h2 className="font-display text-4xl text-white uppercase">TECH_STACK // EMPOWERED_BY_AI</h2>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                     {displayCategories.map((cat, idx) => (
                        <div key={cat.category} className="p-6 md:p-8 bg-[#121212]/50 border border-white/5 relative group hover:border-[#00daf7]/30 transition-all overflow-hidden">
                              ...
                              >
                                 {[...cat.skills, ...cat.skills].map((skill, i) => (
                                    <div key={i} className="flex flex-col gap-1 pr-6">
                                       <span className="font-display text-xl md:text-2xl text-white group-hover:text-[#00daf7] transition-colors cursor-default tracking-tight uppercase">{skill}</span>
                     ...
                     <div className="md:col-span-2 p-8 border border-[#00daf7]/10 bg-[#00daf7]/5 group">
                        <div className="font-mono text-[10px] text-[#00daf7] tracking-[0.4em] mb-6 uppercase">PRIMARY_FOCUS</div>
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                           <span className="font-display text-4xl md:text-6xl text-white group-hover:text-[#00daf7] transition-colors">FRONTEND_DEV</span>
                           <span className="font-mono text-xs text-gray-500 uppercase tracking-[0.2em]">REACT / TS / NEXT.JS</span>
                        </div>
                     </div>
                  </div>
               </div>
...
const TimelineCard = ({ item }: any) => (
   <div className="relative p-8 border border-[#00daf7]/10 bg-[#121212]/40 backdrop-blur-sm group hover:border-[#00daf7]/50 transition-all flex flex-col h-full w-full whitespace-normal">
      <div className="font-mono text-[10px] text-[#00daf7] mb-2">{item.period}</div>
      <h3 className="font-display text-2xl text-white uppercase mb-1 group-hover:text-[#00daf7] transition-colors">{item.role}</h3>
      <div className="font-mono text-xs text-gray-500 italic mb-6 uppercase tracking-widest leading-none">{item.company}</div>
